...
def input_string(gui_id, value, long=False, code=False):
    text_id = str(gui_id);
...
def typed_input(gui_id, T, value, previews=False, tooltip=False):
    read_only = getattr(T, "read_only", False);
    code = getattr(T, "code", False);
    ...
    if isinstance(T, asset_types.List) and getattr(T, "colour", False):
        return list(input_colour(gui_id, value, getattr(T, "palette", None)));
    ...
    if isinstance(T, asset_types.String):
        value = input_string(gui_id, value, code, code);
...
def swatch_grid(gui_id, palette, size=20, cols=8):
    """Returns the colour clicked this frame, or None."""
    picked = None;

    imgui.push_id(str(gui_id));
    for idx, colour in enumerate(palette["colours"]):
        r, g, b = colour["rgb888"];
        if idx % cols != 0:
            imgui.same_line();
        if imgui.color_button(f"##{idx}", imgui.ImVec4(r/255, g/255, b/255, 1), imgui.ColorEditFlags_.no_tooltip, (size, size)):
            picked = (r, g, b);
        if imgui.is_item_hovered():
            imgui.set_tooltip(f"{colour["name"]}\n{r}, {g}, {b}");
    imgui.pop_id();

    return picked;

def input_colour(gui_id, value, palette=None):
    gui_id = str(gui_id);
    palette = AssetManager.search("palette", palette) if isinstance(palette, str) else palette;

    r, g, b = value;
    # the built-in picker can't be handed a palette, so it makes way for one
    flags = imgui.ColorEditFlags_.no_picker if palette != None else 0;
    changed, (r, g, b) = imgui.color_edit3(gui_id, (r/255, g/255, b/255), flags);
    value = int(r*255), int(g*255), int(b*255);

    # both ping the last item, so they belong to the colour widget, not the
    # palette button that follows it
    ContextMenu.ping(gui_id);
    Tooltip.ping();

    if palette != None:
        swatches = [colour["rgb888"] for colour in palette["colours"]];
        if changed:
            value = colours.nearest(swatches, value);

        popup_id = f"palette##{gui_id}";
        imgui.same_line();
        if imgui.button(f"{palette["name"]}##{gui_id}"):
            imgui.open_popup(popup_id);
        if imgui.begin_popup(popup_id):
            picked = swatch_grid(popup_id, palette);
            if picked != None:
                value = picked;
